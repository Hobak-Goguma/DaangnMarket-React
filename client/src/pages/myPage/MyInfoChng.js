...
const MyInfoChng = ({ ID, login, changeLogin, history }) => {
  ...
  const [agreeCh, setAgreeCh] = useState({
    //약관 체크를 위한 state
    SMS: false,
    mail: false,
  });
    ...
    switch (
      ...
      case "newPw": // 새로운 비밀번호 유효성 검사용 클래스로 조정
        ...
        for (let i = 0; i < tmpString.length; i++) {
          if (
            tmpString[i].charCodeAt([0]) > 47 &&
            tmpString[i].charCodeAt([0]) < 58
          ) {
            //숫자
            if (combCh[1] === false) {
              combCh[1] = true;
            ...
            if (
              i < tmpString.length - 2 &&
              tmpString[i].charCodeAt([0]) ===
                tmpString[i + 1].charCodeAt([0]) &&
              tmpString[i].charCodeAt([0]) === tmpString[i + 2].charCodeAt([0])
            ) {
              tmpsameNum = "false";
            }
          } else if (
            tmpString[i].charCodeAt([0]) > 64 &&
            tmpString[i].charCodeAt([0]) < 91
          ) {
            //대문자
            if (combCh[2] === false) {
              combCh[2] = true;
            ...
            tmpString[i].charCodeAt([0]) > 96 &&
            tmpString[i].charCodeAt([0]) < 123
          ) {
            //소문자
            if (combCh[2] === false) {
              combCh[2] = true;
      ...
      case "submit": //변경 버튼 눌렀을때 함수
        if (
          newPwCh.sameNum !== "true" ||
          newPwCh.differPw !== "true" ||
          newPwCh.moreEight !== "true" ||
          newPwCh.combinate !== "true" ||
          newPwCh.newPwClass !== "wrong" ||
          newPwCh.newPwChClass !== "wrong"
        ) {
          ...
          fetch(`www.daangn.site/memeber/${user.pk}`, {
            //수정용 api주소 수정용 fetch
            method: "PUT",
            headers: {
              "Content-type": "application/json",
    ...
    let year =
      type === "year"
        ? value === ""
          ? ""
          : Number(value)
        : Number(birth.year); //연도 입력
    let month =
      type === "month"
        ? value === ""
          ? ""
          : Number(value)
        : Number(birth.month); // 월 입력
    let day =
      type === "day" ? (value === "" ? "" : Number(value)) : Number(birth.day); //일 입력
      ...
      {!user.login ? (
        ...
        <>
          ...
          <div className="btnBox">
            <a href="" className="secession" id="secession" onClick={change}>
              탈퇴하기
            </a>
            <a href="" className="submit" id="submit" onClick={change}>
              회원정보수정
            </a>
          </div>
        </>
